...
// ■記事を1件表示する
// 
// http://127.0.0.1/blog/?action=entry&id=
// 呼び出し元: ../index.php
//======================================================
...
if($entry['記事状態'] != "公開"){
    ...
    $設定['管理用:記事状態の表示'] = str_replace("《記事状態》", $entry['記事状態'], $設定['_管理用:記事状態の表示']);
}
...
if(コメントを表示するなら($entry)){
    ...
    $設定['コメント'] = コメントのHTML作成($comment);
}

//コメントフォーム作成
if(コメントが受付中なら($entry)){
    $設定['コメントフォーム'] = コメントフォームのHTML作成();
}
...
$設定['前の記事'] = ヘッドラインのHTML作成($prev_entry, "←前の記事");
$設定['次の記事'] = ヘッドラインのHTML作成($next_entry, "次の記事→");
