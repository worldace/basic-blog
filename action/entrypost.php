<?php
//======================================================
// ■新規投稿した記事を記録する (管理用、POST)
// 
// http://127.0.0.1/basic-blog/?action=entrypost
// 呼び出し元: ../index.php
//======================================================
...
//ブログテーブルに追加
$設定['記事ID'] = データベース追加("insert into ブログ (記事タイトル, 記事本文, 記事カテゴリ, 記事アイキャッチ画像, 記事サムネイル画像, 記事状態, 記事投稿時間) values (?, ?, ?, ?, ?, ?, {$_SERVER['REQUEST_TIME']})",
array($_POST['title'], $_POST['body'], $_POST['category'], $アイキャッチ画像, $サムネイル画像, $_POST['entry-status']));

//履歴テーブルに追加
データベース追加("insert into 履歴 (記事ID, 履歴投稿時間, 履歴本文) values ({$設定['記事ID']}, {$_SERVER['REQUEST_TIME']}, ?)",
array($_POST['body']));
